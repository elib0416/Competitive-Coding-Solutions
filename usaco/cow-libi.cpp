...
long long int dis(long long int x, long long int y, long long int tx, long long int ty){
...
int main(){
    ...
    for (long long int i = 0; i < q; i++){
        ...
        long long int index = distance(alibis.begin(), lower);
        alibi current;
        ...
        if(index >= 0){
            ...
            if(dis(x, y, current.x, current.y) > (t - current.t) * (t - current.t)){
                //cout << "A" << index << ' ' << x << ' ' << y << ' ' << t << ' ' << current.x << ' ' << current.y << endl;
                innocent++;
                continue;
            }
        }
